...
public class Radar : MonoBehaviour
{
    ...
    [SerializeField, InspectorName("Radar Control Y Axis")]
    InputActionReference radarControlYAxis;
    ...
    void UpdateGimbleRotation()
    {
        ...
        if (radarControlXAxis != null && radarControlXAxis.action != null) xIn = radarControlXAxis.action.ReadValue<float>();
        if (radarControlYAxis != null && radarControlYAxis.action != null) yIn = radarControlYAxis.action.ReadValue<float>();

        xIn = Mathf.Clamp(xIn, -1f, 1f);
        yIn = Mathf.Clamp(yIn, -1f, 1f);

        float localY = xIn * 90f;
        float localX = -yIn * 90f;
    ...
    void UpdateAllScreens()
    {
        ...
        Transform g = gimble != null ? gimble.transform : transform;
